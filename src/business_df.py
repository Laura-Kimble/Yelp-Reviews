    ...
    def plot_stars_hist(self, view_by_col='', limit=10, save=False):
        ...
        if view_by_col:
            ...
            for idx, label in enumerate(np.array(labels)):
                ...
                ax.set_title('Average Star Ratings')
                ax.legend()
        ...
        if save:
            fig.savefig(f'../images/Stars distribution by {view_by_col}.png')


    def plot_review_counts_hist(self, view_by_col='', cutoff=5000, save=False):
        ''' 
        ...
        if view_by_col:
            num_plots = len(cutoff_data[view_by_col].unique())
            fig, axs = plt.subplots(num_plots, 1, sharex=True, figsize=(8, 4 * num_plots))

            for idx, val in enumerate(cutoff_data[view_by_col].unique()):
                data = cutoff_data[cutoff_data[view_by_col]==val]['review_count']
                ax = axs[idx]
                ax.hist(data, bins=20, label=f'{view_by_col} = {val}')
        ...
        if save:
            fig.savefig(f'../images/Review Counts by {view_by_col}.png')
